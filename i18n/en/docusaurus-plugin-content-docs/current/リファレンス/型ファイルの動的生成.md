...
## strictUndefinedChecks (previewFeatures)

By specifying `strictUndefinedChecks` in the `generator` block's `previewFeatures`, you can tighten how `undefined` is handled in query input (equivalent to Prisma's preview feature of the same name).
...
When enabled, passing an **explicit `undefined`** in query input throws a runtime error (`GassmaUndefinedValueError`). To express "do not specify this field", either **omit the key itself** or use `Gassma.skip`.

```ts
// NG: an explicit undefined throws GassmaUndefinedValueError
gassma.User.findMany({ where: { name: undefined } });

// OK: omit the key
gassma.User.findMany({ where: { age: { gte: 20 } } });

// OK: express "not specified" with Gassma.skip
gassma.User.findMany({ where: { age: { gte: 20 }, name: Gassma.skip } });
```

You cannot specify `Gassma.skip` as an array element (`GassmaSkipInArrayError`). Use `null`, or filter out `Gassma.skip` elements beforehand.

When `strictUndefinedChecks` is disabled (omitted), the behavior is as before, and an explicit `undefined` is treated as not specified.

:::tip
To reject `undefined` entirely, we recommend enabling `exactOptionalPropertyTypes` in the consumer's `tsconfig.json` (the same requirement as Prisma). This also detects, at the type level, code that assigns `undefined` to optional properties.
:::
