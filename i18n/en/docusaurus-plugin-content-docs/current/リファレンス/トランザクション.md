...
description: "Commit all writes inside a callback at once with $transaction. On error, not a single cell is written. maxWait / timeout / rollback options"
---
...
- The lock only serializes operations that use GASsma within the same script project. It cannot stop manual edits to the spreadsheet or changes from other scripts that do not use GASsma.
