...
description: "Defining oneToMany, oneToOne, manyToOne, and manyToMany relations between sheets"
---
...
When using the CLI, relations are written in `schema.prisma` with `@relation`. Running `npx gassma generate` injects them into the generated client.
...
When using the GAS editor alone, pass a `relations` option to the `GassmaClient` constructor.
...
The rest of this page uses the constructor form to describe what each relation type means. For how to write relations in the schema, see [Schema](/docs/reference/schema).
