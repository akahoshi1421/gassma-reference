...
## strictUndefinedChecks Errors

These occur when the `strictUndefinedChecks` preview feature is enabled. For details, see [Local Development with Prisma Schema](/docs/reference/type-generation).

| Error | Message | Trigger Condition |
| --- | --- | --- |
| `GassmaUndefinedValueError` | Invalid value for argument \`\{path\}\`: explicitly \`undefined\` values are not allowed. | An explicit `undefined` is specified in query input |
| `GassmaSkipInArrayError` | Invalid value for argument \`\{path\}\`: Can not use \`Gassma.skip\` value within array. Use \`null\` or filter out \`Gassma.skip\` values. | `Gassma.skip` is specified as an array element |
