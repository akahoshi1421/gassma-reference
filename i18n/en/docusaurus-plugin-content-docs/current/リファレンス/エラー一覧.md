...
| `GassmaInvalidValueError` | Invalid value for argument \`\{argumentName\}\`. Expected \{expected\}. | An argument value has an unacceptable shape (e.g., a non-array for `OR`, a `sort` other than `"asc"` / `"desc"` in `orderBy`, a `nulls` other than `"first"` / `"last"`, an empty object for `select` / `cursor` / `select` in `count`, a `where` with no conditions at all in the single-row operations `update` / `delete` / `upsert`, a non-scalar value in `data`, `NaN` / `Infinity` / `-Infinity` / Invalid Date / arrays / functions / Symbols / BigInts as write (`data`) or query (`where` / `cursor` / `having`) values, or `Gassma.raw` as a `where` value) |
