...
There are 54 exported error classes; together with `GassmaClient` / `GassmaController` / `FieldRef` / `skip` / `raw` / `migrateSheets`, the `Gassma` namespace exposes 60 public entities.
