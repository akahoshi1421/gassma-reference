...
| `GassmaInvalidValueError` | Invalid value for argument \`\{argumentName\}\`. Expected \{expected\}. | 引数の値が受け付けられない形（例: `OR` に配列以外、`orderBy` の `sort` に `"asc"` / `"desc"` 以外、`nulls` に `"first"` / `"last"` 以外、`count` の `select` に空オブジェクト、`data` の値にスカラー以外） |
