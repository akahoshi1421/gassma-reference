...
| `GassmaInvalidValueError` | Invalid value for argument \`\{argumentName\}\`. Expected \{expected\}. | 引数の値が受け付けられない形（例: `OR` に配列以外、`orderBy` の `sort` に `"asc"` / `"desc"` 以外、`nulls` に `"first"` / `"last"` 以外、`select`・`cursor`・`count` の `select` に空オブジェクト、単一行操作（`update` / `delete` / `upsert`）の `where` に条件が 1 つもない、`data` の値にスカラー以外、書き込み（`data`）やクエリ（`where` / `cursor` / `having`）の値に `NaN` / `Infinity` / `-Infinity` / Invalid Date / 配列 / 関数 / Symbol / BigInt、`where` の値に `Gassma.raw`） |
