...
## strictUndefinedChecks 系

`strictUndefinedChecks` プレビュー機能を有効にしている場合に発生します。詳しくは[型ファイルの動的生成](/docs/reference/type-generation)を参照してください。

| エラー | メッセージ | 発生条件 |
| --- | --- | --- |
| `GassmaUndefinedValueError` | Invalid value for argument \`\{path\}\`: explicitly \`undefined\` values are not allowed. | クエリ入力に明示的な `undefined` を指定 |
| `GassmaSkipInArrayError` | Invalid value for argument \`\{path\}\`: Can not use \`Gassma.skip\` value within array. Use \`null\` or filter out \`Gassma.skip\` values. | 配列の要素に `Gassma.skip` を指定 |
