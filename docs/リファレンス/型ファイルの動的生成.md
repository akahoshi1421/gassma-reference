...
`generator` ブロックの `previewFeatures` に `strictUndefinedChecks` を指定すると、クエリ入力の `undefined` の扱いを厳格化できます（Prisma の同名プレビュー機能に相当）。
...
有効にすると、クエリ入力に**明示的な `undefined`** を渡した場合に実行時エラー（`GassmaUndefinedValueError`）がスローされます。「その項目を指定しない」ことを表したい場合は、**キー自体を省略する**か、`Gassma.skip` を使用してください。

```ts
// NG: 明示的な undefined は GassmaUndefinedValueError
gassma.User.findMany({ where: { name: undefined } });

// OK: キーを省略する
gassma.User.findMany({ where: { age: { gte: 20 } } });

// OK: Gassma.skip で「指定しない」を表す
gassma.User.findMany({ where: { age: { gte: 20 }, name: Gassma.skip } });
```

配列の要素に `Gassma.skip` を指定することはできません（`GassmaSkipInArrayError`）。`null` を使うか、`Gassma.skip` の要素を事前に取り除いてください。

`strictUndefinedChecks` を無効（省略）にした場合は従来どおりの挙動となり、明示的な `undefined` は指定なしとして扱われます。

:::tip
`undefined` を完全に拒否するには、利用側の `tsconfig.json` で `exactOptionalPropertyTypes` を有効にすることを推奨します（Prisma と同じ制約です）。これにより、省略可能なプロパティに `undefined` を代入するコードが型レベルでも検出されます。
:::
