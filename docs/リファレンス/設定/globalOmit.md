...
他の設定と違い、`omit` はスキーマではなくコンストラクタで指定します。CLI を使う場合も同じ書き方です。

| | 書き方 |
| --- | --- |
| スキーマ（CLI） | スキーマ側の書き方はありません |
| コンストラクタ（[GAS エディタ](/docs/reference/gas-editor)） | `omit: { Users: { password: true } }` |

`omit` に対応するスキーマ属性はありません。CLI を使う場合もコンストラクタで指定してください。
