...
`Gassma` の参照方法は使い方によって変わります。

| | 書き方 |
| --- | --- |
| CLI（生成クライアント） | `import { Gassma } from "./generated/gassma/schemaClient";` |
| [GAS エディタ](/docs/reference/gas-editor) | import せずに素の `Gassma` |
...
公開されているエラークラスは 56 個で、これに `GassmaClient` / `GassmaController` / `FieldRef` / `skip` / `raw` / `migrateSheets` を加えた 62 個が `Gassma` 名前空間の公開実体です。
...
| `GassmaGroupByOrderByRequiredError` | groupBy requires \`orderBy\` when using \{arguments\}. Specify \`orderBy\` with at least one field, or remove \{arguments\}. | `groupBy` で `take` を指定、または `skip` に 0 以外を指定したのに `orderBy` がない。空の `orderBy` は指定したことになりません。\{arguments\} には実際に使った引数名（`take` / `skip`、両方なら `take` and `skip`）が入ります |
...
| `GassmaInvalidLockError` | \`lock\` must be a Lock returned by LockService. LockService.getDocumentLock() returns null in a standalone script or a web app; use LockService.getScriptLock() instead. | `GassmaClient` の `lock` に Lock として使えない値を指定（`LockService.getDocumentLock()` はスタンドアロンスクリプトまたはウェブアプリから呼ぶと `null` を返します） |

## autoincrement 系

詳しくは [autoincrement](/docs/reference/config/autoincrement) を参照してください。

| エラー | メッセージ | 発生条件 |
| --- | --- | --- |
| `GassmaAutoincrementNotConfiguredError` | Field \`\{field\}\` on \`\{sheetName\}\` is not configured with autoincrement. Autoincrement fields on \`\{sheetName\}\`: \{configuredFields\} | `$getAutoincrement` / `$setAutoincrement` / `$syncAutoincrement` に autoincrement を設定していないフィールドを指定 |
| `GassmaAutoincrementInTransactionError` | \`\{methodName\}\` cannot be called inside $transaction. The autoincrement counter lives in ScriptProperties, so it is not rolled back when the transaction fails. Call it outside $transaction. | `$transaction` の中から `$setAutoincrement` / `$syncAutoincrement` を呼び出し（`$getAutoincrement` は読み取りのみなので呼べます） |

`GassmaAutoincrementNotConfiguredError` の 2 文目は、そのシートに autoincrement フィールドが 1 つもない場合は <code>Sheet \`\{sheetName\}\` has no autoincrement fields.</code> になります。
...
| `$setAutoincrement` の `next` が 1 以上の整数でない（`NaN` / `Infinity` / 小数 / 0 以下 / 数値以外） | `next` | an integer greater than or equal to 1 |
| `$syncAutoincrement` で、autoincrement を設定したフィールドの列がシートに存在しない | `field` | a column that exists on \`\{modelName\}\` |
...
| エラー | メッセージ | 発生条件 |
| --- | --- | --- |
| `GassmaTransactionLockTimeoutError` | Transaction API error: Unable to start a transaction in the given time. The maxWait for this transaction was \{maxWaitMs\} ms. | `$transaction` の開始時、`maxWait` 以内にクライアントの `lock` を取得できない |
| `GassmaTransactionLockRequiredError` | $transaction requires a lock. Pass \`lock\` when constructing GassmaClient, for example \`new GassmaClient(\{ lock: LockService.getScriptLock() \})\`. The client generated by \`npx gassma generate\` does this for you. | `lock` を持たないクライアントで `$transaction` を呼び出し |
...
CLI がスキーマを読むときに発生するエラーです。本体のエラーと違い `Gassma` 名前空間からは公開されておらず、ターミナルに表示されるメッセージで判断します。

| エラー | 発生条件 |
| --- | --- |
| `NoModelsError` | スキーマにモデルが 1 つも定義されていない（`generate` / `migrate dev` / `db push`） |
| `UnsupportedAttributeError` | [使えない属性](/docs/reference/schema#使えない属性)（`@@id` / `@@index` / `@@fulltext` / ネイティブ型）を指定（`generate` / `validate`） |
| `CompositeRelationError` | `@relation` の `fields` / `references` に 2 列以上を指定（`generate` / `validate`。[複合外部キー](/docs/reference/schema#複合外部キー)を参照） |
| `IgnoredRelationColumnError` | `@relation` が `@ignore` の付いたフィールドを参照している |
| `ThroughSheetConflictError` | 暗黙的 Many-to-Many の中間シート名が、2 組のモデルで衝突している |

メッセージは以下のとおりです（`{...}` の部分には実際の値が入ります）。
...
**UnsupportedAttributeError**（属性ごとにまとめて報告されます）
...
詳しくは [migrate / db push](/docs/reference/migrate) を参照してください。

| エラー | 発生条件 |
| --- | --- |
| `MigrateOutputDirError` | `--output` も `.clasp.json` の `rootDir` も無く、出力先が決まらない |
| `MigrateConfirmationRequiredError` | 非対話環境で `migrate dev` を実行し、削除の[確認](/docs/reference/migrate#削除の確認migrate-dev)が必要になった |
| `NoMigrationTrailError` | `migrate deploy` を実行したが、証跡が 1 つも記録されていない |
...
`gassma studio` が開くスプレッドシートを解決するときに発生するエラーです。詳しくは [gassma studio](/docs/reference/cli/commands#gassma-studio) を参照してください。

| エラー | 発生条件 |
| --- | --- |
| `NoDatasourceUrlError` | スキーマの `datasource`・`gassma.config.ts` の `datasource.url`・`.clasp.json` の `parentId` のいずれにも URL が無い |
| `InvalidClaspJsonError` | `.clasp.json` が存在するのに、JSON オブジェクトとして読めない |
