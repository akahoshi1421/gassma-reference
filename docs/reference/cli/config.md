...
`.clasp.json` の `parentId` まで見るのは [`gassma studio`](/docs/reference/cli/commands#gassma-studio) だけです。生成されるクライアントに埋め込まれる `id` と、[migrate / db push](/docs/reference/migrate) がスタブに埋め込む `spreadsheetId` は、上の 2 つからのみ解決されます。
