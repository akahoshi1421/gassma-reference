...
GASsma には 2 つの使い方があります。どちらも同じ機能を使えます。

| 使い方 | 始め方 |
| --- | --- |
| **CLI（ローカル開発）** | `schema.prisma` からシートの型を生成し、補完と型チェックの効く状態で書きます → [クイックスタート](/docs/quickstart) |
| **GAS エディタだけ** | ライブラリを追加してすぐ書き始めます → 以下の手順 |

以下では、GAS エディタだけで動かす最短の例を示します。
...
開いたら[導入方法](./installation)を見ながら GASsma のライブラリを追加してください。
...
:::note
ローカルで TypeScript を使って開発する場合は、`schema.prisma` からシートの型を生成できます。シート名・カラム名の補完が効き、誤りがビルド時に分かります。[クイックスタート](/docs/quickstart)を参照してください。
:::
...
なお、数式インジェクションは書き込み時の自動エスケープで防ぎます。意図的に数式を書き込みたい場合は [raw](/docs/reference/raw) を参照してください。
