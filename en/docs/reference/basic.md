...
| `omit` | Global omit settings | The value you pass takes effect | [Global omit](/docs/reference/config/global-omit) |
| `relations` | Relation definitions | Comes from the schema (ignored when passed) | [Relation Definition](/docs/reference/relation/definition) |
| `defaults` | Default values for fields | Comes from the schema (ignored when passed) | [defaults](/docs/reference/config/defaults) |
| `updatedAt` | Auto-update timestamps | Comes from the schema (ignored when passed) | [updatedAt](/docs/reference/config/updated-at) |
| `ignore` | Field-level exclusion | Comes from the schema (ignored when passed) | [ignore](/docs/reference/config/ignore) |
| `ignoreSheets` | Sheet-level exclusion | Comes from the schema (ignored when passed) | [ignore](/docs/reference/config/ignore) |
| `map` | Field name mapping | Comes from the schema (ignored when passed) | [map](/docs/reference/config/map) |
| `mapSheets` | Sheet name mapping | Comes from the schema (ignored when passed) | [map](/docs/reference/config/map) |
| `autoincrement` | Auto-increment | Comes from the schema (ignored when passed) | [autoincrement](/docs/reference/config/autoincrement) |
| `lock` | Lock used by `$transaction` and autoincrement (defaults to `LockService.getScriptLock()`) | The value you pass takes effect | [Lock](/docs/reference/transaction#lock) |

### Schema-Derived Options Override Constructor Arguments

The generated `GassmaClient` constructor overrides the options with the settings read from `schema.prisma`. As a result, the options marked "Comes from the schema" in the table above are **silently ignored — with no error — when passed to the constructor**. Write these settings in `schema.prisma` instead (see [Schema](/docs/reference/schema)).
...
The override only happens in the `GassmaClient` generated by the CLI. When using `new Gassma.GassmaClient()` directly without the CLI, every option takes effect ([Using the GAS Editor](/docs/reference/gas-editor)).

`strictUndefinedChecks` is not a constructor option on the generated client. When using the CLI, enable it via `previewFeatures` in the `generator` block; once enabled, the generated client passes it automatically ([strictUndefinedChecks / Gassma.skip](/docs/reference/config/strict-undefined-checks)).
