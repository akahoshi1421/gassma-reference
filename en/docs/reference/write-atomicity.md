...
`$transaction` acquires a lock, so **writes that go through GASsma are serialized with each other**. The read-then-write window described above can no longer be interrupted by another GASsma write.

This lock only serializes **operations that use GASsma**. It has no effect on:

- **a person editing the spreadsheet by hand**
- **another script writing to the sheet without GASsma**

GASsma cannot do anything about these. If the sheets may be edited by hand while your operations run, the protection has to come from the design side — for example, **arranging things so the sheets are simply not touched concurrently**, or **running your operations during hours when edits are not accepted**.
