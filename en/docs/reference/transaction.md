...
`$transaction` starts only after acquiring the `lock` passed to `GassmaClient`.

The client generated by `npx gassma generate` fills this `lock` with `LockService.getScriptLock()` as the default. That `LockService.getScriptLock()` **is evaluated in your own project**, so the lock it returns is **your script project's lock**. When you use the CLI, nothing changes in how you write your code.

```ts
const gassma = new GassmaClient(); // lock is filled in for you
```

This `lock` is also used when [autoincrement](/docs/reference/config/autoincrement) assigns numbers.

### Lock Granularity

Specifying `lock` in the constructor overrides the default.
...
| Value | Scope that gets serialized |
| --- | --- |
| `LockService.getScriptLock()` (default) | Executions of the same script project |
| `LockService.getDocumentLock()` | Executions on the same document (the bound spreadsheet) |
| `LockService.getUserLock()` | Executions by the same user |

There is no `lock` in the second argument of `$transaction`. The granularity is decided when the client is constructed.

`LockService.getDocumentLock()` **returns `null` when called from a standalone script or a web app** (it does not throw). Even a script bound to a spreadsheet gets `null` when it runs through a web app. Passing `null` as `lock` throws `GassmaInvalidLockError`.

### When No lock Is Passed

Calling `$transaction` on a client that has no `lock` throws `GassmaTransactionLockRequiredError`. When using the GAS editor alone, pass it yourself (see [Using the GAS Editor](/docs/reference/gas-editor)).
...
A transaction starts only after acquiring the client's [`lock`](#lock). If another execution holds the same lock (such as another running `$transaction`), it waits up to `maxWait` milliseconds for the lock to be released, and throws `GassmaTransactionLockTimeoutError` if it still cannot be acquired.
...
- **What gets serialized is determined by the lock you pass as `lock`.** With the default `LockService.getScriptLock()`, only **executions of the same script project** are serialized. When **a different GAS project touches the same spreadsheet, the two transactions are not serialized against each other**. See [Lock](#lock) for choosing the granularity.
...
- `$setAutoincrement` / `$syncAutoincrement`, which rewrite the [autoincrement](/docs/reference/config/autoincrement) counter, cannot be called inside a transaction (`GassmaAutoincrementInTransactionError`). The counter lives in `PropertiesService` and is therefore outside the scope of the rollback. The read-only `$getAutoincrement` can be called.
...
For details on `GassmaTransactionLockTimeoutError` / `GassmaTransactionTimeoutError` / `GassmaNestedTransactionError` / `GassmaTransactionRollbackError` / `GassmaTransactionLockRequiredError` / `GassmaInvalidLockError`, see the [Error List](/docs/reference/errors).
