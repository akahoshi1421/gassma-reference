...
| --- | --- | --- |
| `@id` | Primary key | - |
| `@relation(...)` | Relation definition | [Relation Definitions](/docs/reference/relation/definition) |
...
| `@unique` / `@@unique` | No effect in GASsma (Prisma uses them to validate relations) | [@unique / @@unique](#unique--unique) |

For attributes that are not listed here, see [Unsupported Attributes](#unsupported-attributes).
...
The composite foreign keys required by the last two cases cannot be used in GASsma yet (see [Composite Foreign Keys](#composite-foreign-keys)).

## Unsupported Attributes

The attributes below make `npx gassma generate` / `npx gassma validate` fail with a `GASsmaUnsupportedAttributeError`.

| Attribute | Why it is rejected |
| --- | --- |
| `@@id` | A composite primary key cannot be declared on a spreadsheet. Prisma never requires `@@id` -- a single `@id` always covers it -- and GASsma's [implicit Many-to-Many](#implicit-many-to-many) assumes the join columns are named `id`, so a composite primary key would point at a column that does not exist. |
| `@@index` / `@@fulltext` | An index cannot be created on a spreadsheet. Every query reads the whole sheet either way, so it has no effect. |
