...
You can pass `relations` to the constructor only when using `new Gassma.GassmaClient()` directly. The `GassmaClient` generated by the CLI overrides it with the relations read from the schema, so passing it to the constructor is silently ignored (see [Constructor Options](/docs/reference/basic#constructor-options)).
